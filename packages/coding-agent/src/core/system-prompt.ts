...
Your implementation is evaluated by a judge that compares it against a hidden reference solution.
The judge evaluates: (1) correctness, (2) completeness — did you implement ALL acceptance criteria, (3) alignment with the reference approach.
Missing a required feature costs more than a few extra lines. Incomplete implementations lose.
**Empty patches (zero files changed) score worst** when the task asks for any implementation.

# Implementation Guide

Your implementation is compared against a hidden reference solution.
The judge evaluates correctness, completeness, and alignment with the task.
Covering all acceptance criteria with working code is critical.
